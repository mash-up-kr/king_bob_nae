...
import com.example.king_bob_nae.features.intro.signup.certification.presentation.SignUpCheckDialog
...
class CheckCertificationFragment :
                ...
                introViewModel.result.collect { authResponse ->
                    if (authResponse.checkCertification) {
                        requireView().findNavController()
                            .navigate(
                                R.id.action_checkCertificationFragment_to_resetPasswdFragment,
                            )
                    } else {
    ...
    private fun showDialog(email: String, type: TYPE) {
        ...
        dialog.show(requireActivity().supportFragmentManager, "ResetCheckDialog")
    }
}